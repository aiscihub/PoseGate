...
def _measure_trace(
    ...
    *,
    ...
    digest_prefix: bool,
) -> tuple[np.ndarray, list[FrameGeometry], str | None]:
    alignment, ligand = _selections(universe, config)
    dt_ns = _frame_interval_ns(universe)
...
def measure_prefix(
    ...
    *,
    ...
    times, geometries, prefix_hash = _measure_trace(
        universe, config, frame_limit=required, digest_prefix=True
    )
...
def measure_late_outcome(
    ...
    times, geometries, _ = _measure_trace(
